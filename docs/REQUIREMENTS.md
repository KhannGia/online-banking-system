...
| 3.2 | Withdraw at maturity | `SC:183` `withdrawAtMaturity` — interest from `_computeInterest` (`SC:349`), principal from SavingCore, interest from the vault | `SavingCore.test.ts:200` |
| 3.3 | Early withdrawal | `SC:208` `earlyWithdraw` — zero interest, penalty from the snapshot to `feeReceiver` | `SavingCore.test.ts:305` |
| 3.4 | Manual renew | `SC:248` `renewDeposit(depositId, newPlanId)` — compounds interest into new principal, adopts the **new** plan's rates, requires target plan enabled (`SC:260`) | `SavingCore.test.ts:347` |
| 3.5 | Auto renew | `SC:283` `autoRenewDeposit(depositId)` — permissionless, after `maturityAt + GRACE_PERIOD` (`SC:291`), **locks the original APR**, mints to the original owner | `SavingCore.test.ts:448` |

Interest formula (assignment §3.2), `SC:349`:
...
| 2 | Simple interest only, no intra-term compounding | `_computeInterest` (`SC:349`) — a single closed-form calculation |
| 3 | Early withdrawal gives zero interest; penalty → `feeReceiver` | `SC:208` (emits `Withdrawn` with `interest = 0`) |
| 4 | Auto-renew preserves the original APR | `SC:283` passes `d.aprBpsAtOpen`, never reads the current plan |
| 5 | Interest always paid from the vault | Every interest path calls `VM:64` `payInterest` through `_payInterestSafe` (`SC:174`); principal never funds interest. **Improved by bonus C1** (below) rather than reverting on shortfall **or** on a paused vault |
| 6 | Paused ⇒ no withdrawals or renewals | `whenNotPaused` on every SavingCore entry point; `VM:64` `payInterest` is separately pausable, but a paused vault no longer blocks SavingCore's own withdraw/renew paths — see `_payInterestSafe` (`SC:174`) |
...
| 7.2 Test suite, coverage > 90% | **81 tests passing.** Coverage: MockUSDC 100/100/100/100, VaultManager 100/100/100/100, SavingCore 100 stmts / 95.45 branches / 100 funcs / 100 lines |
...
| earlyWithdraw: correct penalty, no interest | `SavingCore.test.ts:305` |
| renewDeposit: correct new principal, status update | `SavingCore.test.ts:347` |
| autoRenewDeposit: before grace fails, after grace, APR locked | `SavingCore.test.ts:448` + `:461` (exact `>=` boundary) |
| Vault: fund, withdraw, insufficient vault for payout | `VaultManager.test.ts:8`, `:93`; shortfall in the C1 blocks (`SavingCore.test.ts:254`, `:399`, `:545`) |
| Pause: withdraw blocked when paused | `SavingCore.test.ts:134` + paused cases in each flow's describe; vault-paused-not-underfunded cases at `:285`, `:432`, `:592` |
...
| 2 | Empty vault | Bonus C1 — `VM:64` `payInterest` never reverts on shortfall; wrapped in `try/catch` (`_payInterestSafe`, `SC:174`) so a **paused** vault degrades the same way; shortfall recorded as `pendingInterest`, claimable via `SC:234` `claimInterest` |
| 3 | Dead bot | Bonus G — `SC:283` `autoRenewDeposit` is permissionless and pays a keeper reward. Reference bot: `contract/scripts/keeper-bot.ts` (`npm run keeper:sepolia`) |
| 4 | Rounding dust | `Math.mulDiv` floors; the remainder is never created as an obligation, so it stays in the vault and can never cause a revert. Test: `SavingCore.test.ts:636` |
| 5 | Boundary times | `SC:191` `>=` maturity (withdraw), `SC:216` `<` maturity (early), `SC:257` `>=` maturity (renew, no upper bound), `SC:291` `>=` maturity+grace (auto-renew). Exact-second tests: `SavingCore.test.ts:461` |
| 6 | Disabled plan with active deposits | `SC:260` blocks manual renew **into** a disabled plan; auto-renew deliberately never reads `enabled`; withdraw/claim unaffected |
| 7 | Attack thinking | Reentrancy via `_safeMint` → `onERC721Received`; stopped by `nonReentrant` **and** CEI status ordering (`SC:197`, `SC:222`, `SC:266`, `SC:297` — status set before every external call). Test: `SavingCore.test.ts:614` with `contracts/test/MaliciousReceiver.sol` |
...
|---|---|---|---|
| **C1** | Principal is always safe | `VM:64` `payInterest` pays `min(amount, balance)` and never reverts on shortfall; every caller in `SC` goes through `_payInterestSafe` (`SC:174`), which also catches a **paused**-vault revert the same way; principal is always returned first; shortfall → `pendingInterest`; NFT not burned; `SC:234` `claimInterest` collects later, repeatably | Underfunded vault: `SavingCore.test.ts:254` (withdraw), `:399` (renew), `:545` (auto-renew). Paused vault (same guarantee, different cause): `:285`, `:432`, `:592` |
...
| **G** | Permissionless keeper incentive *(own idea)* | `SC:283` callable by anyone; reward = `mulDiv(interest, keeperRewardBps, 10000)` paid from the vault **on top of** the user's interest; user's interest has priority (paid first, structurally); reference bot at `contract/scripts/keeper-bot.ts` | `SavingCore.test.ts:448`, incl. a partial-funding case proving priority |
...
|---|---|---|
| Correct interest & penalty math | 20 | `SC:349`; worked examples verified in README §4 and in tests |
| APR/penalty snapshot immutability | 15 | `SC:39` + snapshot test |
| Auto-renew with APR lock & grace period | 15 | `SC:283`, `GRACE_PERIOD = 3 days` (`SC:23`), boundary tests `SavingCore.test.ts:461` |
