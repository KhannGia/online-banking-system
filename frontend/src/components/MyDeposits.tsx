...
import { RenewableDeposits } from './RenewableDeposits'
...
export function MyDeposits({ onChanged }: { onChanged: () => void }) {
  ...
  return (
    <div className="space-y-4">
      {isLoading ? (
        <div className="text-ink-500 py-12 text-center">Loading your deposits…</div>
      ) : isError ? (
      ...
      {/* Auto-renewing is permissionless, so this shows regardless of whether the
          connected wallet owns any deposits of its own — see RenewableDeposits.tsx. */}
      <RenewableDeposits onChanged={onChanged} />
    </div>
  )
}
